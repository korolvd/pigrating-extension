...
// syncReward (одиночная награда): prompt параметризуется (фильм), без зачистки сирот
let mvBody;
...
const mv = await syncReward({ clientId: 'c', token: 't', broadcasterId: 'b' }, { rewardId: '', rewardTitle: 'Предложить фильм', cost: 1, points: 0, target: 'input', prompt: 'Напиши название фильма' });
eq('syncReward: prompt награды фильма параметризован + input + cost 1', { id: mv.id, prompt: mvBody && mvBody.prompt, input: mvBody && mvBody.is_user_input_required, cost: mvBody && mvBody.cost }, { id: 'mv1', prompt: 'Напиши название фильма', input: true, cost: 1 });
