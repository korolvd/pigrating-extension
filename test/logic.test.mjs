...
import { parseCsv, colToIndex, buildPlan, executePlan, resolveChoice } from '../core.js';
...
eq('plan: Nik → update в l1 (fastId+investor)', { a: plan[0].action, id: plan[0].lotId, f: plan[0].fastId, inv: plan[0].investor }, { a: 'update', id: 'l1', f: 1, inv: '[СОЦ] Nik' });
...
eq('resolve → кандидат = update', (() => { const r = resolveChoice(multiItem, 'l3', '[СОЦ] '); return { a: r.action, id: r.lotId, f: r.fastId, t: r.target, inv: r.investor }; })(), { a: 'update', id: 'l3', f: 3, t: 'kedi', inv: '[СОЦ] MULTI' });
eq('resolve → new = create с приставкой', (() => { const r = resolveChoice(multiItem, 'new', '[СОЦ] '); return { a: r.action, t: r.target, inv: r.investor }; })(), { a: 'create', t: '[СОЦ] MULTI', inv: '[СОЦ] MULTI' });
...
const markedLots = [
  { id: 'm1', fastId: 9, name: 'Half-Life 2', amount: 907, investors: ['2BeFirefly', '[СОЦ] 2BeFirefly'] },
  { id: 'm2', fastId: 8, name: 'Solo', amount: 50, investors: ['solo', '[СОЦ] other'] },
];
...
const execPlan = [
  { nick: 'Nik', points: 10, action: 'update', fastId: 1, target: 'Эрго прокси', investor: '[СОЦ] Nik' },
